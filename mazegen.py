...
class MazeGenerator:
    ...
    def __init__(self, width: int, height: int,
        ...
        self.maze: List[List[dict]] = self.create_grid()
    ...
    backtracking, skipping reserved cells."""
        ...
        for x, y in self.reserved_cells:
            self.maze[y][x] = {"up": False, "right": False, "down": False, "left": False}
        # Walls to remove in each direction
        directions = [
                    {"x": 0, "y": -1, "current": "up",    "opposite": "down"},
                    {"x": 1, "y": 0,  "current": "right", "opposite": "left"},
                    {"x": 0, "y": 1,  "current": "down",  "opposite": "up"},
                    {"x": -1, "y": 0, "current": "left",  "opposite": "right"},
                     ]
        visited = []
        ...
        def explore(x: int, y: int) -> None:
            ...
            for direction in directions:
                ...
                if (neighbor_x < 0 or neighbor_x >= self.width or
                    ...
                    continue
                if (visited[neighbor_y][neighbor_x] or
                        (neighbor_x, neighbor_y) in self.reserved_cells):
                    continue
